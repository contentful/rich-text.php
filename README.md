...
| [CONTRIBUTING.md](./CONTRIBUTING.md) | Development setup, workflow, PHP version matrix, release process |
| [Architecture Decisions](./docs/ADRs/) | Why things look the way they do |
| [Active Specs](./docs/specs/) | Implementation-level specs for current/recent work |

**Key invariants (full list in [AGENTS.md](./AGENTS.md)):**
- Always use `parseLocalized($data, $locale)` — not the deprecated `parse()`.
- `CatchAll` must use `appendNodeRenderer()`, never `pushNodeRenderer()`.
- `EmbeddedImage` renderer is opt-in — call `$renderer->enableEmbeddedImageRenderer(true)` to activate.
- Public interface changes (`ParserInterface`, `RendererInterface`, etc.) require a major version bump.
